...
class AdminController extends Controller
{
    ...
    public function showResourceUpdate($id)
    {
        $resource = Resource::find($id);
        return view('update-resource', compact('resource'));
    }
    ...
    public function updateResource(Request $request, $id)
    {
        ...
        if ($resource->name != $request->name) {
            $this->validate($request, [
                'name' => ['required', 'string', 'max:255', new UniqueNameInResourceGroup($resource->resource_group_id)]
            ]);
        }
        
        $resource->name = $request->name;
        $resource->cost = $request->cost;
        $resource->uThreshold = $request->threshold;
