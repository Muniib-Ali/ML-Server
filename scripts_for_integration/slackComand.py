...
while True:
    ...
    for user, usage in cpu_data.items():
        if user not in users_to_exclude and usage['CPU'] > resources.get(user) and user not in [booking['resource_name'] for booking in relevant_bookings]:
            message = f"A user is using {user}: without booking and appointment! and threshold is {resources.get(user)}"
